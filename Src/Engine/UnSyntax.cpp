...
{
	return toupper(c[0]) + toupper(c[1]) * 13;
}
struct FKeyHash
{
	FKeyHash *Next;
...
void RtfTopicHandler::Get( ULevel *Level, const char *Item, FOutputDevice &Out )
{
	...
	if( Text && Text->Num>0 )
	{
		...
		int iLine=1;
		const char *Stream = &Text->Element(0);
		// Info: 329 328
		char Line[2048];
		int CommentCount=0;
		while( *Stream != 0 )
		{
			...
			if( *Stream != '#' )
			{
				End = Line + sprintf( Line, "%s", CommentCount ? RTF_GREEN : RTF_WHITE );
			}
			else
			{
				IsLiteral = 1;
				End = Line + sprintf( Line, "%s", CommentCount ? RTF_GREEN : RTF_GREY );
			}
			for( ; ; )
			{
				// Detect keywords.
				if( CommentCount==0 && !IsQuote && !IsLiteral )
				{
					...
					{
						if( !isalnum(*Stream) && *Stream!='_')
						{
							// Got end of word.
							*End = 0;
							if( FirstWord && *Stream==':' && stricmp(Word,"default")!=0 )
							{
								// Label.
								char Temp[256];
								strcpy(Temp,Word);
								End = Word + sprintf(Word,"%s%s%s", RTF_YELLOW, Temp, RTF_WHITE );
							}
							else for( FKeyHash *Item=Hash[CalcHash(Word)]; Item; Item=Item->Next )
							{
								if( stricmp(Word,Item->Key)==0 )
								{
									// Found a keyword to syntax highlight.
									char Temp[256];
									strcpy(Temp,Word);
									End = Word + sprintf(Word,"%s%s%s", RTF_CYAN, Temp, RTF_WHITE );
									break;
								}
							}
							Word      = NULL;
				...
				{
					if( IsQuote )	End += sprintf(End,"\"" RTF_WHITE);
					else			End += sprintf(End,RTF_GREEN "\"");
					IsQuote = !IsQuote;
					Stream++;
				}

...
=========================================================================
FILENAME:     UnAudio.h
DESCRIPTION:  Include file for modules that call the intermediate level
              Unreal sound engine (UnAudio.lib or equivalent).
...
/*
The following functions are the library's "public" functions
that can be called by the higher level sound code in Unreal.
Refer to the block comments in se*.c for details about specific
functions.
*/
...
int	SoundEngSoundPlayEx(int sid, int volume, int pan, double pitchm);
void	SoundEngSoundStop(int pid);
void	SoundEngSoundChange(int pid, int newvol, int newpan);
void	SoundEngSoundSetRadius(int pid, double radius);
void	SoundEngSoundScaleVolume(int pid, double scale);
...
int	SoundEngMusicSpecifySong(char *data, int is_file);
void	SoundEngMusicGetSongName(char *name, int maxchrs);
char *	SoundEngMusicGetSongPtr(void);
...
int	SoundEng16BitFlagSet(int val);
int	SoundEng16BitFlagGet(void);

int	SoundEngSurroundFlagSet(int val);
int	SoundEngSurroundFlagGet(void);

void	SoundEngReverbParamsSet(int space, int depth);

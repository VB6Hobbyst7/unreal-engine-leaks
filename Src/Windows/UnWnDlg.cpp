...
{
	guard(CUnrealWnDlg::OnInitDialog);
	...
	if( GetDWORD(App.CmdLine,"HWND=", &ParentWindow) )
		::SetParent(m_hWnd,(HWND)ParentWindow);
...
//
void CUnrealWnDlg::OnCancel()
{
	guard(CUnrealWnDlg::OnCancel);
...
//
void CUnrealWnDlg::OnOK()
{
	guard(CUnrealWnDlg::OnOK);
...
// may terminate.
//
void CUnrealWnDlg::Exit()
{
	guard(CUnrealWnDlg::Exit);
	EndDialog(IDOK);
	PostQuitMessage(0);
	unguard;
}
...
{
	guard(CUnrealWnDlg::OnQueryDragIcon);
...
{
	guard(CUnrealWnDlg::OnSysCommand);
	...
	{
		// Another message we don't handle.
...
{
	guard(CUnrealWnDlg::Log);

	int TopEditLine,NumLines,LastLineIsOnScreen,LastChar;

	if( m_CmdLog.GetLineCount()>MAX_LINES )
	{
		// Delete starting stuff.
...
{
	guard(CUnrealWnDlg::OnShowWindow);
...
// Show the dialog.
//
void CUnrealWnDlg::ShowMe()
{
	guard(CUnrealWnDlg::ShowMe);
